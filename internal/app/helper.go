...
const (
	// recommendedHomeDir 定义放置 base 服务配置的默认目录.
	recommendedHomeDir = ".base"

	// defaultConfigName 指定了 base 服务的默认配置文件名.
	defaultConfigName = "base.yaml"
)
...
func initConfig() {
	...
	// 读取环境变量的前缀为 Base，如果是 base，将自动转变为大写。
...
// initStore 读取 db 配置，创建 gorm.DB 实例，并初始化 base store 层.
